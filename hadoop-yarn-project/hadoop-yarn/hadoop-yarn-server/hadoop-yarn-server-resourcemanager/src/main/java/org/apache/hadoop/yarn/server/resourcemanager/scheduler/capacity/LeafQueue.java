...
public class LeafQueue extends AbstractCSQueue {
  ...
  //this hash  table is used to record the suspend time of app
  Queue<ApplicationAttemptId> suspendedApps = 
	  new LinkedList<ApplicationAttemptId>();
  ...
  public void submitApplication(ApplicationId applicationId, String userName,
    ...
    // Inform the parent queue，如果没有问题的话才可以提交application，仅仅是说明可以在这个queue上提交，但是并没有说具体什么时候真正的可以run
    try {
  ...
  public void finishApplication(ApplicationId application, String user) {
    ...
    LOG.info("finish Application "+application.getId()+" by "+user);
      ...
      synchronized (application) {
        ...
        for (Priority priority : application.getPriorities()) {
          ResourceRequest anyRequest =
              application.getResourceRequest(priority, ResourceRequest.ANY);//为什么先分配any的
          if (null == anyRequest) {
            continue;
          }
          ...
          // Do we need containers at this 'priority'?， 有这个优先级的请求，但是暂时请求的containers数目为0
    ...
    
 {
  ...
  boolean canAssign(FiCaSchedulerApp application, Priority priority, 
    ...
    if (type == NodeType.OFF_SWITCH) {
      ...
      // 'Delay' off-switch，如果没有strict限制，则所有的请求都是off switch
      ResourceRequest offSwitchRequest = 
          application.getResourceRequest(priority, ResourceRequest.ANY);
      long missedOpportunities = application.getSchedulingOpportunities(priority);
      //随着程序的执行，requiredContainers数目会逐渐的降低，该调度但是没调度则该opportunity就会增加
    ...
    if (type == NodeType.RACK_LOCAL) {
      ...
      long missedOpportunities = application.getSchedulingOpportunities(priority);
      return (
    	  //如果miss的大于了configure的node locality delay，则我们在该节点分配。	  
  ...
  private Resource assignContainer(Resource clusterResource, FiCaSchedulerNode node, 
    ...
    if (availableContainers > 0) {
      ...
      if (rmContainer != null) {
      //之前reserve过container，现在开始使用这个reserve的containers
        unreserve(application, priority, node, rmContainer);
