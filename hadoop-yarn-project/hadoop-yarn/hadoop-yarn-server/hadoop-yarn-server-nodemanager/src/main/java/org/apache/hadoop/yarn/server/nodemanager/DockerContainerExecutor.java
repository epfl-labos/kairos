...
public class DockerContainerExecutor extends ContainerExecutor {
  ...
  public int launchContainer(Container container,
    ...
    //  }
      LOG.info("[[PAMELA INITIATE DOCKER shell command "+sb.getWrapperScriptPath().toString()+" containerIdStr "+containerIdStr +" userName "+userName+" pidFile "+pidFile);
      LOG.info("[[PAMELA COMMANDS in script " + commandStr);
      LOG.info("[[PAMELA containerWorkDir " + containerWorkDir);
  ...
  public void writeLaunchEnv(OutputStream out, Map<String, String> environment, Map<Path, List<String>> resources, List<String> command) throws IOException {
    ...
    if (environment != null) {
    	 String tempKey = "CLASSPATH";
    	 String tempValue=null; 
    	
      for(Map.Entry<String,String> env : environment.entrySet()) {
          if(env.getKey().toString().equals(tempKey)){
        	  tempValue=env.getValue().toString()+":$SPARK_HOME/lib/*";  
          }	
      }
      
      if(tempValue!=null){
         
    	 environment.put(tempKey, tempValue);
